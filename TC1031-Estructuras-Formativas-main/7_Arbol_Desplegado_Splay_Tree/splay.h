...
Tarea7: Árbol Desplegado o Splay Tree 
*/
...
Node<T>* Node<T>::remove(T val) {
    ...
    Node<T>* deletion = find(val);
    Node<T>* parent = deletion->parent;
    Node<T>* node;

    if (deletion->left == 0 && deletion->right == 0) {
        if (parent) {
            if (parent->left == deletion) {
                parent->left = 0;
            } else {
                parent->right = 0;
            }
    ...
    else if (deletion->left == 0 || deletion->right == 0) {
    	if (deletion->left != 0) {
        	node = deletion->left;
    	} else {
        	node = deletion->right;
